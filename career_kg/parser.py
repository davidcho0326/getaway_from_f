...
# Source files — centralized in career_kg/data/
DATA_DIR = Path(__file__).resolve().parent / "data"
SOURCE_DIR = DATA_DIR / "sources"
SOURCE_FILES = {
    "resume": SOURCE_DIR / "resume_fin.md",
    "portfolio": SOURCE_DIR / "project_portfolio.md",
    "strategy": SOURCE_DIR / "st.md",
    "essays": SOURCE_DIR / "greenhouse_essays.md",
    "jd": SOURCE_DIR / "jd.md",
    "cj_enm": SOURCE_DIR / "cj_enm_portfolio.md",
    "saas_vendor": SOURCE_DIR / "saas_vendor_experience.md",
    "gaps": DATA_DIR / "candidate_gaps.md",
}

OUTPUT_FILE = DATA_DIR / "career_data.json"
...

{
  ...
  "projects": [
    {
      ...
      "category": "claude_mastery | enterprise_ai | ecosystem | content_business",
  ...
  "skills": [
    {
      "id": "skill_slug",
      "name": "string",
      "category": "AI/LLM | Production | Development | Cloud | Data | Language | Business | Marketing"
    }
  ],
  "achievements": [
    {
  ...
  "candidate_gaps": [
    {
      "id": "gap_slug — e.g. gap-b2b-sales",
      "requirement_id": "jd_slug — references jd_requirements[].id",
      "gap_description": "string — what is missing or weak relative to the JD requirement",
      "mitigation": "string — how the candidate compensates for this gap",
...
- Category mapping:
  ...
  - content_business: CJ ENM digital content business, B2B sales, brand partnerships (CJ ENM portfolio)
- Extract ALL candidate gaps from the gap analysis document
  - Each gap must reference one of the 6 jd_requirements by ID
  - Include both the gap and the mitigation strategy
  - Severity: high = critical gap, medium = notable but compensated, low = minor or well-mitigated
  - Also extract general weaknesses as gaps with requirement_id "general"
"""
...
def extract_from_sources(llm: LLMClient) -> dict:
    ...
    cj_enm = read_source("cj_enm")
    saas_vendor = read_source("saas_vendor")
    gaps = read_source("gaps")
...
=== CJ ENM PORTFOLIO (cj_enm_portfolio.md) ===
{cj_enm}

=== SAAS VENDOR EXPERIENCE (saas_vendor_experience.md) ===
{saas_vendor}
...
=== CANDIDATE GAP ANALYSIS (candidate_gaps.md) ===
{gaps}
"""
...
"""
    ...
    response = llm.generate_response(prompt, system_instruction=EXTRACTION_SCHEMA, max_tokens=32768)
...
def validate_data(data: dict) -> list[str]:
    ...
    if len(gaps) < 3:
        issues.append(f"Only {len(gaps)} candidate gaps (expected 6+)")
...
def print_summary(data: dict):
    ...
    print(f"  Candidate Gaps:  {len(data.get('candidate_gaps', []))}")
    print("─" * 40)
